export class SeoChecker {
    async inspectPage(page) {
      ...
      return {
        ...
        // OpenGraph (15 pts)
        let ogScore = 0;
        if (data.openGraph.hasOgTitle)
            ogScore += 5;
        else
            issues.push('Missing OpenGraph title (og:title).');
        if (data.openGraph.hasOgDesc)
            ogScore += 5;
        else
            issues.push('Missing OpenGraph description (og:description).');
        if (data.openGraph.hasOgImage)
            ogScore += 5;
        else
            issues.push('Missing OpenGraph preview image (og:image).');
        score += ogScore;
        if (ogScore < 15) {
