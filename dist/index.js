...
async function main() {
    ...
    if (options.image)
        ...
        focus = 'api';
    else if (options.status || options['404'])
        focus = 'status';
    else if (options.seo || options.meta)
        focus = 'seo';
    else if (options.speed || options.perf)
        focus = 'speed';
        ...
        if (focus === 'seo') {
            ...
            for (let i = 0; i < urlsToAudit.length; i++) {
                ...
                try {
                    ...
                    if (res.seoMetadata)
                        seoResults.push(res.seoMetadata);
        ...
        if (focus === 'speed') {
            ...
            for (let i = 0; i < urlsToAudit.length; i++) {
                ...
                try {
                    const res = await auditor.runAudit();
                    if (res.pageSpeed)
                        speedResults.push(res.pageSpeed);
...
async function runMultiPageAudit(pagesToAudit, targetUrl, sitemapSourceUrl, focus, options) {
    ...
    for (let i = 0; i < pagesToAudit.length; i++) {
        ...
        try {
            ...
            pageSpinner.succeed(`[${i + 1}/${pagesToAudit.length}] Checked: ${pageResult.pageTitle || pageUrl} (${pageResult.summary.errorCount} errors, ${pageResult.summary.warningCount} warnings)`);
