...
- 📁 **CLI Report Importer**: Drag-and-drop or load any `audit_*.json` generated by the CLI directly into the Web UI for visual inspection.
- 🎯 **Pre-Configured Demo Presets**: Test instantly with Asian Paints E-Commerce, Nutrien Ag Solutions, Broken Demo Suite, Clean Fast Site, and Localhost dev profiles.
...
```text
? Enter Webpage URL, Domain, Sitemap XML, or File Path to audit (or type "clean" to clear reports): https://betabeautifulhomes.asianpaints.com/
? Select audit diagnostic for https://betabeautifulhomes.asianpaints.com/: (Use arrow keys)
