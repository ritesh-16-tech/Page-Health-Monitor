...
import { ImageIssue } from '../types/audit.js';

export class ImageChecker {
  private networkImageStatuses: Map<string, { status: number; statusText: string }> = new Map();
  ...
  async checkImages(page: Page): Promise<ImageIssue[]> {
    ...
    const uniqueRaw: typeof rawImages = [];
    ...
    const issues: ImageIssue[] = await Promise.all(
      uniqueRaw.map((raw) =>
        limit(async () => {
