...
import { PageSpeedResult, SpeedBottleneck } from '../types/audit.js';

export class SpeedChecker {
  async inspectSpeed(page: Page): Promise<PageSpeedResult> {
    const pageUrl = page.url();

    const perfData = await page.evaluate<any>(`(() => {
    ...
    let score = 100;
    const bottlenecks: SpeedBottleneck[] = [];
    ...
    if (score < 55) rating = 'Poor';
    else if (score < 80) rating = 'Needs Improvement';
