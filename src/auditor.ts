...
export class PageAuditor {
  ...
  async runAudit(onProgress?: (stage: string, detail?: string) => void): Promise<AuditResult> {
    ...
    let pageStatus: PageStatusResult | undefined;
    if (focus === 'all' || focus === 'status') {
      if (onProgress) onProgress('status', `Checking page HTTP status and redirect chain for ${targetUrl}...`);
    ...
    try {
      if (onProgress) onProgress('launch', 'Launching browser engine...');
      ...
      if (onProgress) onProgress('navigate', `Navigating to ${targetUrl}...`);
      ...
      if (this.options.scroll) {
        if (onProgress) onProgress('scroll', 'Auto-scrolling page to trigger lazy-loaded assets & dynamic APIs...');
        await this.autoScroll(page);
        await page.waitForTimeout(1500);
      }

      // 2. Check SEO Metadata
      let seoMetadata: SeoMetadataResult | undefined;
      if (focus === 'all' || focus === 'seo') {
        if (onProgress) onProgress('seo', 'Inspecting SEO metadata, headings, Schema JSON-LD & social cards...');
      ...
      if (focus === 'all' || focus === 'speed') {
        if (onProgress) onProgress('speed', 'Measuring Core Web Vitals, Navigation Timing & payload sizes...');
        pageSpeed = await speedChecker.inspectSpeed(page);
      }

      // 4. Check Images
      let images: ImageIssue[] = [];
      if (focus === 'all' || focus === 'image') {
        if (onProgress) onProgress('images', 'Analyzing DOM and network images...');
      ...
      let fonts: FontIssue[] = [];
      if (focus === 'all') {
        if (onProgress) onProgress('fonts', 'Inspecting WebFonts & font declarations...');
      ...
      if (focus === 'all' || focus === 'link') {
        if (onProgress) onProgress('links', 'Extracting and verifying hyperlinks & redirections...');
        links = await linkChecker.checkAllLinks(page, finalPageUrl, (checked, total, current) => {
          if (onProgress) onProgress('links_progress', `Checking links (${checked}/${total}): ${current.slice(0, 50)}...`);
        });
      }
      ...
      const failedNetworkApis = networkTraffic.filter(
        (n) => (n.resourceType === 'fetch' || n.resourceType === 'xhr') && n.isFailed
      ).length;
