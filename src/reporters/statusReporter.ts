...
export class StatusReporter {
  ...
  static printTerminal(
    ...
    console.log(
      `${chalk.bold('Status Stats:')}  ${chalk.green.bold(`200 OK: ${count200}`)} | ${chalk.yellow.bold(`3xx Redirects: ${count3xx}`)} | ${chalk.red.bold(`404 Not Found: ${count404}`)} | ${chalk.red.bold(`5xx Errors: ${count5xx}`)} | ${chalk.magenta.bold(`Other Errors: ${countOther}`)}`
    );
    ...
    for (const item of displayList.slice(0, 25)) {
      ...
      if (item.is404 || item.httpStatus >= 500 || item.httpStatus === 0) statusColor = chalk.red.bold;
      else if (item.isRedirect) statusColor = chalk.yellow.bold;
    ...
    if (htmlPath) console.log(`${chalk.bold('Interactive HTML Dashboard:')} ${chalk.green.underline(htmlPath)}`);
  ...
  private static renderCsv(results: PageStatusResult[]): string {
    const rows: string[] = [];
    rows.push([
      'Page URL',
      'HTTP Status',
  ...
  private static renderHtml(results: PageStatusResult[], targetUrl: string): string {
    const total = results.length;
    const count200 = results.filter((r) => r.httpStatus === 200).length;
    const count3xx = results.filter((r) => r.isRedirect).length;
    const count404 = results.filter((r) => r.is404).length;
...
<body>
  ...
  private static renderSummaryCsv(results: PageStatusResult[], targetUrl: string): string {
    const total = results.length;
    const count200 = results.filter((r) => r.httpStatus === 200).length;
    const count3xx = results.filter((r) => r.isRedirect).length;
    const count404 = results.filter((r) => r.is404).length;
  ...
  private static renderSummaryHtml(results: PageStatusResult[], targetUrl: string): string {
    const total = results.length;
    const count200 = results.filter((r) => r.httpStatus === 200).length;
    const count3xx = results.filter((r) => r.isRedirect).length;
    const count404 = results.filter((r) => r.is404).length;
