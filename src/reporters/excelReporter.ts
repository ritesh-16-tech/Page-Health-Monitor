...
import { AuditResult, MultiPageAuditResult } from '../types/audit.js';
...
export class ExcelReporter {
  ...
  static async generateMultiPage(result: MultiPageAuditResult, outputDir = './reports'): Promise<string> {
    const targetDir = getFunctionalReportDir(outputDir, result.focus || 'all');
    await fs.mkdir(targetDir, { recursive: true });
