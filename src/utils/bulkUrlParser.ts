...
export class BulkUrlParser {
   ...
   */
  static async parseInput(input: string): Promise<string[]> {
  ...
  static parseCsvContent(content: string): string[] {
    ...
    let urlColIndex = headers.findIndex((h) =>
      h.includes('url') || h.includes('link') || h.includes('page') || h.includes('target') || h.includes('href')
    );
   ...
   * Parse JSON content
   */
  static parseJsonContent(content: string): string[] {
    try {
   ...
   */
  static parseTextContent(content: string): string[] {
  ...
  private static cleanUrlCandidate(val: string): string | null {
  ...
  private static deduplicateUrls(urls: string[]): string[] {
